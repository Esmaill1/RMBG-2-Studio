...
| **Dual Backend** | CPU-optimized (`flask_app.py`) and GPU-accelerated (`flask_app_gpu.py`) |
...
| Aspect | `flask_app.py` (CPU) | `flask_app_gpu.py` (GPU) |
|--------|----------------------|--------------------------|
...
RMBG-2-Studio/
├── app/
│   ├── flask_app.py          # CPU-optimized Flask server
│   ├── flask_app_gpu.py      # GPU-accelerated Flask server
│   ├── requirements.txt      # CPU dependencies
│   ├── requirements-gpu.txt  # GPU dependencies (includes torch+torchvision)
│   ├── static/
...
### GPU Backend Configuration (`flask_app_gpu.py`)
...
5. Install application dependencies from `app/requirements.txt` or `app/requirements-gpu.txt`
...
# Or run the GPU server
python app/flask_app_gpu.py
```
...
5. **[5/7] Install PyTorch (CPU)** — Installs from `https://download.pytorch.org/whl/cpu`
...
- Entry point: `python flask_app_gpu.py`
...
The GPU backend (`flask_app_gpu.py`) implements several advanced optimizations:
...
- **Cause**: `torch.cuda.is_available()` returns `False`
- **Fix**: Ensure NVIDIA drivers and CUDA toolkit are installed, or use `flask_app.py` (CPU version) instead
